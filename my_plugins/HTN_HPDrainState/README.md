...
ターンが終わるたびにHPを吸収されるステート（状態異常）を作成できるようになります。

MPドレインについては  
**[HTN_MPDrainState](https://github.com/nekonenene/RPG-Maker-MZ-plugins/tree/main/my_plugins/HTN_MPDrainState)** をお使いください。

## 🛠️ 導入方法

**[【ここを右クリックして「名前を付けてリンク先を保存」みたいな項目を選んでダウンロード】](https://raw.githubusercontent.com/nekonenene/RPG-Maker-MZ-plugins/main/my_plugins/HTN_HPDrainState/HTN_HPDrainState.js)**
...
ステートの「メモ」欄に `<HPDrainState>` というタグを記入することで、  
...
`<HPDrainState>` が設定されたステートを付与された側（ドレインされる側）は、  
自分のターンが終わるたびに一定量のHPが失われ、そのHPがステートを付与してきた相手（ドレインする側）に渡されます。
...
ターン終了時にドレイン実行者全員からHPを吸収され、ステートを付与した全員が倒された際に、ステートは自動解除されます。

吸収量の計算タイプや吸収量は、プラグインパラメータで設定できます。  
プラグインパラメータの設定値がデフォルトとなりますが、  
ステートの「メモ」欄にタグを記入することで、ステートごとに設定を上書きすることもできます。

ステートの「解除条件」の「戦闘終了時に解除」にはチェックを入れておいてください。  
誰が付与したかの情報は戦闘中しか保持されないため、仮にステートを持ち越したとしても、次の戦闘ではドレインが発生しません。
...
| 値 | 計算方法 |
|---|---|
| `absolute` | 固定値（指定したHP量がそのまま吸収される） |
| `selfMaxHp` | ドレインされる側の最大HPに対するパーセンテージ |
| `selfHp` | ドレインされる側の現在HPに対するパーセンテージ |
| `drainerMaxHp` | ドレインする側の最大HPに対するパーセンテージ |
| `drainerMissingHp` | ドレインする側の「最大HP − 現在HP」（失ったHP）に対するパーセンテージ |
...
「ドレインされる側の最大HPの10分の1」と「ドレインする側の攻撃力の2倍」のどちらか大きい方の値が吸収量になります。
...
- `<HPDrainState_AmountType: タイプ>`  
  吸収タイプを上書き（`absolute` / `selfMaxHp` / `selfHp` / `drainerMaxHp` / `drainerMissingHp` / `formula`）  
- `<HPDrainState_Amount: 数値または式>`  
  吸収量を上書き。タイプが `absolute` なら固定HP量、割合系なら 0〜100 のパーセンテージ、`formula` なら JavaScript 式  
- `<HPDrainState_AmountRandomizer: 数値>`  
  吸収量のランダム幅（%）を上書き（0〜80。例: `20` と設定すると計算結果に 0.8〜1.2 倍のランダムな乗数が掛かる）  
- `<HPDrainState_DrainMessage: テキスト>`  
  HP吸収時のメッセージを上書き（`%1` はドレインされる側の名前、`%2` はドレインする側の名前、`%3` は「ＨＰ」の表示名、`%4` は吸収量に置換される）  
- `<HPDrainState_AllowKill: true/false>`  
  吸収によってドレインされる側を戦闘不能にすることを許可するか上書き（`false` にすると最低 1HP を保持する）
- `<HPDrainState_MultiDrainer: true/false>`  
  複数のドレイン実行者を許可するか上書き（`true` にすると、ステートが治る前に別のキャラから再付与された場合、複数のキャラからHPドレインがされるようになる。付与者全員が倒された際にステートが自動解除される）

#### コピーしやすい用の一覧

```
<HPDrainState>
<HPDrainState_AmountType: selfMaxHp>
<HPDrainState_Amount: 10>
<HPDrainState_AmountRandomizer: 0>
<HPDrainState_DrainMessage: %1は%2に%3を %4 吸収された！>
<HPDrainState_AllowKill: true>
<HPDrainState_MultiDrainer: false>
```
...
例１： **ドレインする側のHPが満タンになる以上は吸わない**

```
# プラグインパラメータに書く場合
Math.min(drainer.mhp - drainer.hp, drainTarget.mhp * 0.1)

# ステートのメモ欄に書くタグの場合
<HPDrainState_Amount: Math.min(drainer.mhp - drainer.hp, drainTarget.mhp * 0.1)>
```

ドレインされる側の最大HPの10%を吸いますが、  
ドレインする側の「最大HP − 現在HP」（失ったHP）を吸収量の上限とするようになります。
...
# ステートのメモ欄に書くタグの場合
<HPDrainState_Amount: 30 + drainer.mat * 2 - drainTarget.mdf * 2>
```
...
# ステートのメモ欄に書くタグの場合
<HPDrainState_Amount: 30 + Math.max(drainer.mat * 2 - drainTarget.mdf * 2, 0)>
```
