...
このプラグインは、最後に状態異常を付与した相手にHPを吸収させる仕様になっていて、  
１人のキャラに対して複数の敵が同時にHPドレインをする状態は作れません。

もし作りたい場合は、手間はかかりますが、  
「吸血（モンスターA）」「吸血（モンスターB）」「吸血（モンスターC）」のように  
ステートをコピーして複数作成し、  
それらのステートを別々のモンスターから付与されるようにすれば、  
複数のモンスターからHPドレインを受けている状態は実現可能です。
