...
1. `ForcedSkillState_Rate` の確率で、スキルを強制使用するかの判定をおこない、強制使用なら次のステップへ
2. `ForcedSkillState_Skill` に書かれたスキルIDのうち、`-1` など存在しないスキルIDを除いたものから１つを選ぶ
3. `ForcedSkillState_Skill` に有効な候補がない場合、 `ForcedSkillState_SkillName` に書かれたスキル名のうち、存在するものから１つを選ぶ
4. `ForcedSkillState_SkillName` にも有効な候補がない場合、元の行動がそのままおこなわれる
5. 有効なスキルがあった場合は、それの使用が強制される。MPやTPの不足、スキル封印、スキルを覚えているかなどの通常の使用条件は無視され、また、別のステートによって混乱状態であっても、スキルの範囲はスキル本来のものが反映される
