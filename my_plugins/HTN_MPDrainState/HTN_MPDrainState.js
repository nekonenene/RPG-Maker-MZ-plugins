...
/*:
 * @target MZ
 * @plugindesc Creates a state that drains MP from the afflicted to the drainer each turn (v1.0.3)
...
/*:ja
 * @target MZ
 * @plugindesc MPを吸収され続けるステート（状態異常）を作成できます (v1.0.3)
...
(() => {
  ...
  const calcDrainAmount = (state, drainTarget, drainer) => {
    ...
    let amount = String(state.meta.MPDrainState_Amount ?? paramAmount).trim().replace(/['"]/g, '').replace(/&lt;/g, '<').replace(/&gt;/g, '>');
  ...
  Game_Battler.prototype.onBattleEnd = function() {
    ...
    delete this._mpDrainerInfo;
    delete this._mpDrainPendingMessages;
    delete this._mpDrainPendingPopups;
  };
})();
