...
| 逃走失敗時の逃走成功率の上昇量(%) | 逃走に失敗するたびに逃走確率に加算される量 | 10 |
...
倒した敵の数が多いほど逃走確率にボーナスが加算されます。

最終逃走確率 = 基本逃走確率 + 撃破した敵の数 × ボーナス率

例えば「ボーナス率 = 10%」のとき、敵を 2 体倒していれば `2 × 10 = 20%` が加算されます。

### 逃走失敗時の上昇量

逃走に失敗するたびに、「逃走失敗時の逃走成功率の上昇量」が逃走確率に加算されます。
デフォルトは 10% で、これはツクールMV/MZのもともとの挙動と同じです。
0% に設定すると、何度失敗しても逃走確率が上昇しなくなります。
