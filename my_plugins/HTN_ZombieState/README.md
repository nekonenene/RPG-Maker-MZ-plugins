...
`setHp()` や `setMp()` が直接呼び出されてHP・MPが増加する場合、回復の反転は正しく機能しますが、  
ダメージポップアップの表示やダメージ音の再生はされません。  
また、 `setTp()` が直接呼び出されてTPが増加する場合、回復の反転は正しく機能しますが、  
バトルログには表示されません。

戦闘不能バトラーへの `setHp(1)`（蘇生）は回復の反転の対象外です。
