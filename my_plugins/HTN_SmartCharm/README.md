...
`<SmartCharm>` が設定されたステートが複数あり、アクターがそれらのステートに同時にかかっている場合、  
「優先度」がもっとも高いステートに書かれているタグが優先されます。
...
- `<SmartCharm_AllowMagic: true/false>`  
  魔法スキル（スキルタイプ１番）の使用可否を上書き
- `<SmartCharm_AllowSpecial: true/false>`  
  必殺技スキル（スキルタイプ２番）の使用可否を上書き
...
### 「魔法スキルの許可」「必殺技スキルの許可」オプションについて

「魔法スキルの許可 (AllowMagic)」は、スキルタイプのIDが 1 であるか、  
「必殺技スキルの許可 (AllowSpecial)」は、スキルタイプのIDが 2 であるかで使用可否を判定しています。

そのため、データベースの「タイプ」＞「スキルタイプ」の設定を初期状態から変更して、  
「01」を魔法系、「02」を必殺技系から変更している場合には、このオプションは想定と異なる挙動をします。ご注意ください。
